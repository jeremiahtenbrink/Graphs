...
class Edge(object):
    ...
    def __init__(self, weight, start_vertex: Node, target_vertex: Node):
        self.weight = weight
...
class BellmanFord(object):
    ...
    def calculate_shortest_path(self, vertex_list: List[Node],
                                edge_list: List[Edge],
                                start_vertex: Node):
        ...
        for i in range(0, len(vertex_list) - 1):
            for edge in edge_list:
                ...
                print(f'index {i}')
                print(f'starting vertext min distance {u.min_distance}')
                print(f'edge weight {edge.weight}')
                print(f"edge: {u.name} -> {v.name}")

                if new_distance < v.min_distance:
                    print(
                        f'{v.name}: Setting new min distance: {new_distance}')

                    v.min_distance = new_distance
                    v.predecessor = u
                else:
                    print(f'{v.name}: keeping min distance: {v.min_distance}')

        for edge in edge_list:
            if self.hash_cyle(edge):
    ...
    def hash_cyle(edge):
        if (
                edge.start_vertex.min_distance + edge.weight) < edge.target_vertex.min_distance:
            return True
        else:
            return False

    @staticmethod
    def get_shortest_path_to(target_vertex: Node):

        if not BellmanFord.HAS_CYCLE:
            print("Shortest path exists with value: ",
                  target_vertex.min_distance)
